...
This is pretty barebones for now. It currently only supports the Ising model using the Metropolis-Hastings and Glauber algorithms. Ultimately it will receive data binning and averaging, file outputs, and more models, algorithms, and observables. My hope is that this will be a good tool for easily simulating different classical models (Ising, XY, Potts, etc.) on different geometries. It would also be nice to add support for next-nearest-neighbor or other bond formats, with adjustable interaction strength.  
